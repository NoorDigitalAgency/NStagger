...
namespace Stagger
{
    public partial class EnglishTokenizer : Tokenizer
    {
        ...
        public override List<Token> ReadSentence()
        {
            ...
            while ((token = Tokenize()) != null)
            {
                ...
                else
                {
                    if (!sentence.IsEmpty())
                    {
                        if (lastNonSpace != null && lastNonSpace.Value.EndsWith(".") && lastNonSpace.Value.Length > 1 && token.IsCapitalized)
                        {
                            Pushback(Length);
                        ...
                        {
                            if (lastNonSpace == null || lastNonSpace.Value.Length != 1 || !lastNonSpace.IsCapitalized)
                            {
                                sentence.Add(token);
                    ...
                    if (token.Type == TokenType.Latin)
                    {
                        ...
                        if (textLower.EndsWith("n't"))
                        {
                            ...
                            {
                                sentence.Add(new Token(TokenType.Latin, token.Value.Substring(0, 3), token.Offset));

                                sentence.Add(new Token(TokenType.Latin, token.Value.Substring(2), token.Offset + 2));
                            }
                            else
                            {
                                sentence.Add(new Token(TokenType.Latin, token.Value.Substring(0, length - 3), token.Offset));

                                sentence.Add(new Token(TokenType.Latin, token.Value.Substring(length - 3), token.Offset + length - 3));
                            }
                        }
                        else
                        {
        ...
        public EnglishTokenizer(TextReader reader)
        {
            this.reader = reader;
        }
